...
struct StarNet2503081200TVAConfig {
    /** 模型运行设备 */
    ...
    /** 模型的配置结构体 */
    StarNet2503081200Options model_config;
};


class StarNet2503081200TVA {
public:
    /// @brief StarNet2503081200TVA 的构造函数
    /// @param config StarNet2503081200TVA 的配置结构体
    StarNet2503081200TVA(const StarNet2503081200TVAConfig& config);
...
private:
    ...
    /** 模型 */
    StarNet2503081200 model = nullptr;
...
class StarNet2503081200TVABuilder {
public:
    /// @brief 设置模型是否运行于 GPU
    /// @param value 运行于 GPU 的标志
    /// @return 当前构建器对象，支持链式调用
    StarNet2503081200TVABuilder& setCUDA(bool value);

    /// @brief 设置训练时期数。
    /// @param value 训练时期数。
    /// @return 当前构建器对象，支持链式调用。
    StarNet2503081200TVABuilder& setEpochs(int value);

    /// @brief 设置热身时期数。
    /// @param value 热身时期数。
    /// @return 当前构建器对象，支持链式调用。
    StarNet2503081200TVABuilder& setWarmup(int value);

    /// @brief 设置批次大小。
    /// @param value 批次大小。
    /// @return 当前构建器对象，支持链式调用。
    StarNet2503081200TVABuilder& setBatchSize(int value);

    /// @brief 设置学习率。
    /// @param value 学习率。
    /// @return 当前构建器对象，支持链式调用。
    StarNet2503081200TVABuilder& setLearningRate(double value);

    /// @brief 设置 Adam 优化器的 beta1。
    /// @param value Adam 优化器的 beta1。
    /// @return 当前构建器对象，支持链式调用。
    StarNet2503081200TVABuilder& setBeta1(double value);

    /// @brief 设置 Adam 优化器的 beta2。
    /// @param value Adam 优化器的 beta2。
    /// @return 当前构建器对象，支持链式调用。
    StarNet2503081200TVABuilder& setBeta2(double value);

    /// @brief 设置 Adam 优化器的 weight decay。
    /// @param value Adam 优化器的 weight decay。
    /// @return 当前构建器对象，支持链式调用。
    StarNet2503081200TVABuilder& setWeightDecay(double value);

    /// @brief 设置原始特征目录
    /// @param value 设定值
    /// @return 当前构建器对象，支持链式调用
    StarNet2503081200TVABuilder& setRootOrig(std::string value);

    /// @brief 设置包络特征目录
    /// @param value 设定值
    /// @return 当前构建器对象，支持链式调用
    StarNet2503081200TVABuilder& setRootEnve(std::string value);

    /// @brief 设置 MUAP 峰峰值图特征目录
    /// @param value 设定值
    /// @return 当前构建器对象，支持链式调用
    StarNet2503081200TVABuilder& setRootMUAPP2P(std::string value);

    /// @brief 设置常见特征目录
    /// @param value 设定值
    /// @return 当前构建器对象，支持链式调用
    StarNet2503081200TVABuilder& setRootComm(std::string value);

    /// @brief 设置保存目录。
    /// @param value 保存目录。
    /// @return 当前构建器对象，支持链式调用。
    StarNet2503081200TVABuilder& setSavePath(std::string value);

    /// @brief 设置验证折。
    /// @param value 验证折。
    /// @return 当前构建器对象，支持链式调用。
    StarNet2503081200TVABuilder& setValFold(int value);

    /// @brief 设置特征组合
    /// @param value 设定值
    /// @return 当前构建器对象，支持链式调用
    StarNet2503081200TVABuilder& setCombination(int value);

    /// @brief 创建并返回 StarNet2503081200TVA 实例。
    /// @param  
    /// @return 新创建的 StarNet2503081200TVA 对象的智能指针。
    std::shared_ptr<StarNet2503081200TVA> build(void);
    
    /** 训练程序的配置结构体 */
    StarNet2503081200TVAConfig config;
