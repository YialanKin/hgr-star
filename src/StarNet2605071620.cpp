...
StarNet2605071620Impl::StarNet2605071620Impl(StarNet2605071620Options model_config) {
...
torch::Tensor StarNet2605071620Impl::forward(torch::Tensor x) {
    int b = x.size(0);
    int c = x.size(1);
    int d = x.size(2);
    int h = x.size(3);
    ...
    x = torch::stft(                    // [b * c * h * w, d] -> [b * c * h * w, 33, 8]
        x,
        /*n_fft=*/64,
        /*hop_length=*/19,
        /*win_length=*/64,
    ...
    x = torch::abs(x);  // 取幅度谱
    x = x.index({torch::indexing::Slice(), torch::indexing::Slice(1, 33), torch::indexing::Slice()});  // 去除零频分量 [b * c * h * w, 33, 24] -> [b * c * h * w, 32, 8]
    int f = x.size(1);
    int t = x.size(2);
    x = x.reshape({b * c, h * w, f, t});    // [b * c * h * w, 32, 8] -> [b * c, h * w, 32, 8]
    x = pre_conv->forward(x);               // e.g. [b * c, h * w = 128, 32, 8] -> [b * c, h * w = 32, 32, 8]
    x = x.permute({0, 3, 2, 1});
    x = pre_bn->forward(x);
    x = pre_act->forward(x);
...
void StarNet2605071620Options::update(void) {
    ...
    head_config = torch::nn::LinearOptions(8 * embed, num_classes);
    return;
}

int StarNet2605071620Options::Depth(int stage) {
    return depths[stage - 1];
}

StarNet2605071620Options& StarNet2605071620Options::setInChannels(int value) {
    in_channels = value;
    update();
    return *this;
}

StarNet2605071620Options& StarNet2605071620Options::setCompressedChannels(int value) {
    compressed_channels = value;
    update();
    return *this;
}

StarNet2605071620Options& StarNet2605071620Options::setEmbed(int value) {
    embed = value;
    update();
    return *this;
}

StarNet2605071620Options& StarNet2605071620Options::setConvKs(int value) {
    conv_ks = value;
    update();
    return *this;
}

StarNet2605071620Options& StarNet2605071620Options::setConvStride(int value) {
    conv_stride = value;
    update();
    return *this;
}

StarNet2605071620Options& StarNet2605071620Options::setDropRate(double value) {
    drop_rate = value;
    update();
    return *this;
}

StarNet2605071620Options& StarNet2605071620Options::setNumClasses(int value) {
    num_classes = value;
    update();
    return *this;
}

StarNet2605071620Options& StarNet2605071620Options::setDepths(std::vector<int> value) {
    depths = value;
    update();
    return *this;
}